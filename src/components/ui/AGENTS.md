...
- `dropdown-menu.tsx`는 Radix DropdownMenu 기반이 아닌 커스텀 구현 - 키보드 네비게이션 미지원
- `sonner.tsx`는 `useTheme` from `next-themes`를 import하지만 앱에 ThemeProvider가 존재하지 않음 - 토스트 테마 감지가 항상 "system"으로 기본값 설정. `userSettings.colorScheme`에서 테마를 파생하도록 개선 필요
